 ...
 *  z.array() rejects (Array.isArray returns false). Non-finite values (NaN, ±Infinity)
 *  are converted to null since Zod rejects them and downstream code already skips nulls.
 *  LaserScan ranges use ±Infinity for no-return beams, so this must cover Infinity, not
 *  just NaN — otherwise a single out-of-range beam fails the whole message.
 *
...
export function normalizeCborMessage(value: unknown): unknown {
  ...
  // TypedArrays (Float32Array, Float64Array, etc.) → plain arrays with non-finite → null
  if (ArrayBuffer.isView(value)) {
    return Array.from(value as Float32Array, (v) => (Number.isFinite(v) ? v : null));
  }
