 ...
 * @prop label - Short label for the connection type (e.g., "ROS").
 * @prop connected - Whether the connection is active.
 */
      ...
      <div className="flex items-center gap-1.5">
        <span
          className={`size-1.5 rounded-full ${
            connected ? 'bg-status-nominal motion-safe:animate-pulse' : 'bg-status-critical'
          }`}
          aria-hidden="true"
        />
        <span
          className={`font-mono text-xs ${connected ? 'text-status-nominal' : 'text-status-critical'}`}
        >
          {connected ? 'OK' : 'OFF'}
        </span>
      </div>
