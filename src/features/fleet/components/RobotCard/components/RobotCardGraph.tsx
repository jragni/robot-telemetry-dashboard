 ...
 * @prop graph - ROS graph data, or null if not yet fetched.
 * @prop isConnected - Whether the robot is currently connected.
 */
export function RobotCardGraph({ graph, isConnected }: RobotCardGraphProps) {
  ...
  return (
    <dl className="flex flex-col gap-2">
      ...
      <RobotCardDataRow
        label="Services"
        value={isConnected && graph ? String(graph.services) : dash}
      />
      <RobotCardDataRow
        label="Actions"
        value={isConnected && graph ? String(graph.actions) : dash}
      />
    </dl>
  );
}
