...
describe('useWebRtcStream/constants', () => {
  ...
  it('falls back to public TURN when only VITE_TURN_URL is set', async () => {
    vi.stubEnv('VITE_TURN_URL', 'turn:my-turn.example.com:443');
    // No username or credential set
    ...
    // Should use free public TURN (3 servers) since credentials are incomplete
    expect(turnServers).toHaveLength(3);
    const firstTurn = turnServers[0];
    expect(firstTurn).toBeDefined();
    expect(getFirstUrl(firstTurn as unknown as RTCIceServer)).toContain('openrelay.metered.ca');
  });
