...
test.describe('UAT — EPIC/bug-hunt-mobile-ux against the cloudflare robot', () => {
  ...
  test('Connect to real robot, verify workspace + pilot + mobile viewport', async ({ page }) => {
    ...
    // 1. Add robot via the modal — UAT path that hits Wave A's modal changes (T-153/T-156/T-152).
    await page.goto('fleet');
    await page
    ...
    // 3. Discovery completes within ~10s of connecting (T-153 empty defaults rely on this).
    ...
    // 5. Sustain data flow briefly so T-104 coalescing actually runs on the hot path,
    //    then sample telemetry rendering signals.
    ...
    // 6. Mobile viewport reflow check (Wave B is unfinished but baseline must hold).
    ...
    // 8. Reset viewport, sustain telemetry briefly so T-104 coalescing has run on the hot path.
